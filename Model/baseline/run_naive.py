...
def rolling_eval(series, split_end, horizon, model_fn):
    ...
    while t + pd.Timedelta(days=horizon - 1) <= eval_end:
        # full history up to t-1 (nguyen tac 3: train <= t)
        train  = series[:t - pd.Timedelta(days=1)].values.astype(float)
...
for h in HORIZONS:
    ...
    for sid in series_ids:
        ...
        r = rolling_eval(series_dict[sid], VAL_END, h, naive_fn)
        for k in scores: scores[k].append(r[k])
        details.append({"model": "Naive", "store": store, "product": product,
                        "horizon": h, **{k: round(float(r[k]), 4) for k in r}})
    row = {"model": "Naive", "dataset": "retail_inventory_daily", "target": TARGET,
           "horizon": h,
...
pd.DataFrame(results).to_csv(f"{RESULT_DIR}/naive_daily_summary.csv", index=False)
pd.DataFrame(details).to_csv(f"{RESULT_DIR}/naive_daily_details.csv", index=False)
print(f"\nSaved summary to {RESULT_DIR}/naive_daily_summary.csv")
print(f"Saved details to {RESULT_DIR}/naive_daily_details.csv")
